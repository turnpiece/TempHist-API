...
def main():
    ...
    print("\n📊 Results:")
    print("   Total job creation attempts: 100")
    ...
    if len(unique_jobs) == 1:
        ...
        print("   All 100 attempts returned the same job_id")
        ...
        print("   Deduplication working but not perfect")
    else:
        print(f"\n❌ FAILURE: {len(unique_jobs)} different jobs created")
        print("   Deduplication not working effectively")
    ...
    for slug, location in locations:
        params = {"scope": "yearly", "slug": slug, "identifier": "11-12", "year": 2024, "location": location}
        job_ids = []
        for _ in range(25):
    ...
    print("   Total attempts: 100 (25 per location)")
    print("   Expected unique jobs: 4 (one per location)")

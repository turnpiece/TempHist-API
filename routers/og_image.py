...
def _render_chart(share: dict, records: list, show_title: bool = True) -> bytes:
    ...
    pairs = [
        (r["year"], r["temperature"]) for r in records if r.get("temperature") is not None and r.get("year") is not None
    ]
    if not pairs:
...
async def _fetch_records_live(share: dict, redis_client) -> Optional[list]:
    ...
    try:
        ...
        records = [v for v in data.get("values", []) if v.get("year") is not None and v.get("temperature") is not None]
        if records:
            logger.info(

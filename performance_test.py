...
def profile_calculation_functions():
    ...
    sample_data_large = [{"x": year, "y": 15.0 + (year - 1970) * 0.1 + (year % 3) * 0.5} for year in range(1970, 2025)]
...
def profile_memory_usage():
    ...
    # Sample data
    sample_data = [{"x": year, "y": 15.0 + (year - 1970) * 0.1} for year in range(1970, 2025)]
    ...
    for func_name, func in [
        ...
        gc.collect()
        memory_before = sum(
            sys.getsizeof(obj) for obj in gc.get_objects() if isinstance(obj, (int, float, str, list, dict))
        )

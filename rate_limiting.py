        ...
        try:
            ...
            if not location_exists:
                ...
                if count >= self.locations_per_hour:
                    return (
                        False,
                        f"Service token location limit exceeded: {count}/{self.locations_per_hour} unique locations per hour",
                    )
...
class LocationDiversityMonitor:
    ...
    def check_location_diversity(self, ip: str, location: str) -> Tuple[bool, str]:
        ...
        if len(unique_locations) > self.max_locations:
            ...
            return (
                False,
                f"Too many different locations ({len(unique_locations)} > {self.max_locations}) in {self.window_hours} hour(s)",
            )
...
class RequestRateMonitor:
    ...
    def check_request_rate(self, ip: str) -> Tuple[bool, str]:
        ...
        total_requests = sum(
            count for timestamp, count in self.ip_requests[ip].items() if float(timestamp) >= window_start
        )
